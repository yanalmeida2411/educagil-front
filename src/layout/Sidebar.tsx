...
import TeacherProfile from '@/components/ui/Teacher/TeacherProfile';
...
export default function Sidebar() {
  ...
  const { full_name} = useUserInfoFromCookies();
    ...
    const { isTeacherModalOpen, setIsTeacherModalOpen, isMobileMenuOpen, setIsMobileMenuOpen } = useComponentsStore()
    ...
    const switchPages = () => {
        if (isMobileMenuOpen && !isStudentModalOpen) {
    ...
    return (
        <>
            <header className="bg-white shadow-md flex items-center justify-between px-8 py-4 max-md:px-4 relative">
                <div className="flex-shrink-0">
                    <img
                        src="assets/svg/educAgilPrafessores.svg"
                        alt="Logo Professores EducAgil"
                        className="w-32 hidden md:block"
                    />

                    <img
                        src="assets/svg/educAgilProfessoresMini.svg"
                        alt="Logo mobile"
                        ...
                        return (
                            <Link
                                ...
                                className={`
                                        ...
                                        ${isActive ? 'text-[#0092BA] font-semibold' : 'text-[#6D6D6D]'}
                                        hover:text-[#0092BA] transition-colors
                                    `}
                            >
                                {icon}
                ...
                <div className="hidden md:flex items-center gap-6 ">
                    <div className="flex items-center gap-3 cursor-pointer "
                        onClick={() => setIsTeacherModalOpen(!isTeacherModalOpen)}>
                        <p className="flex items-center justify-center h-10 w-10 rounded-full bg-[#0092BA] text-white font-medium hover:text-[#0092BA] hover:bg-[#6d6d6d86]">
                            {full_name ? full_name.charAt(0).toUpperCase() : 'E'}
                        </p>
                        <p className="whitespace-nowrap font-medium text-[#333] hover:text-[#0092BA]" >{full_name || 'EducaAgil'}</p>
                    </div>

                    <div className="flex items-center gap-4 text-[#6D6D6D] cursor-pointer ">
                        <MdNotificationsNone id='returnLoginDesktop' className="w-6 h-6 hover:text-[#0092BA]" />
                ...
                {isMobileMenuOpen && (
                    <div className="fixed top-0 left-0 w-full h-full bg-white z-50 flex flex-col px-6 py-6 animate-fadeIn">
                        <div className="flex justify-between items-center mb-6">
                            ...
                            <button onClick={() => setIsMobileMenuOpen(false)} className="text-2xl text-gray-700">
                                <IoClose />
                            </button>
                        </div>
                        ...
                        <div className="flex items-center gap-3 mb-4 cursor-pointer" onClick={() => {
                            ...
                            <p className="text-[#ec0000] font-medium ">{full_name || 'Educagil'}</p>
                        </div>
                            ...
                            {links.map(({ id, href, label, icon }) => {
                                ...
                                return (
                                    <Link
                                        ...
                                        href={href}
                                        id={id}
                                        className={`flex items-center gap-3 p-3 rounded-md ${isActive ? 'bg-[#E8F8FC] text-[#0092BA]' : 'bg-white text-[#444]'
                        ...
                        >
                            <IoLogOutOutline id='returnLoginMobile' className="w-5 h-5" />
                            Sair
                        </Link>
                    </div>
                )}
            </header>
            {isTeacherModalOpen && <StudentProfile />}
        </>
    )
}